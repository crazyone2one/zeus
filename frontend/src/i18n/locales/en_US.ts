const en_US = {
  ...
  license: {
    ...
    invalid: 'invalid',
    count_warning:
      'The number of system users has exceeded the license limit. Please contact the staff as soon as possible.',
    expired: 'expired',
  ...
  workstation: {
    ...
    sync_case_tips:
      'Note: After ignoring, the affected use cases will no longer have reminders, please operate with caution',
    batch_sync_api_tips:
      'Batch synchronization, some use case parameters that do not need to be synchronized may also be overwritten, causing the test to fail, please operate with caution',
    batch_ignore_case_tips:
      'Note: After batch ignoring, the affected use cases will no longer have reminders, please operate with caution',
  },
  display: {
    title: 'Theme',
    logo: 'System LOGO',
  ...
  organization: {
    ...
    message: {
      ...
      webhook: 'Webhook',
      notes:
        '1. In order to use WeChat Enterprise, Dingding and Feishu notifications, you need to create a custom robot and copy the webhook address;<br/>' +
  ...
  load_test: {
    ...
    ramp_up_tips:
      'The RampUp is too large, the chart rendering will not be displayed, this does not affect the test execution, please refer to the actual results',
    iterate_num: 'Iterations',
    by_iteration: 'By iterations',
    by_iteration_tip:
      'The execution time needs to be configured according to the number of iterations. JMeter will decide whether to end the test according to the number of iterations and the execution time',
  ...
  api_test: {
    ...
    definition: {
      ...
      recipient: 'Recipient',
      recipient_tips:
        'When the API changes, the associated CASE creator and automation scene creator will receive in-site messages, can go to',
      select_comp: {
        no_data: 'No Data',
        add_data: 'Add Data',
      },
  ...
  system_parameter_setting: {
    ...
    from_tip:
      'Note: It must be the mailbox verified by the mail server, otherwise it is sent by SMTP account by default',
  },
  i18n: {
    home: 'Home',
  ...
  run_mode: {
    ...
    retry_message:
      'Retry the interface use case or ui case, if it still fails after retries n times, the use case failed',
    retry: 'Retry',
  ...
  ui: {
    ...
    y: 'ordinate',
    click_tip_2:
      'The upper left corner of the default element is 0, 0; by setting the relative position, control the click position of the mouse on the element',
    click: 'click',
    dclick: 'Double click',
    press: 'press',
    standup: 'Bounce up',
    ...
    wait_tip:
      'For the Text attribute of the element, it refers to the text content displayed on the page, and the waiting timeout time is 15000ms',
    ...
    validate_tips:
      'To judge whether the actual result is consistent with the expected one, you can add multiple assertions',
    ...
    title_tip:
      'Assert whether the title of the current window is consistent with the expected value, if it matches exactly, the assertion succeeds, otherwise it fails',
    ...
    param_required_tip:
      'After debugging and saving, the usage of variables will be automatically verified. The parameters used in user-defined steps must be filled in; Not required is the redundant parameter not used in the custom step.',
    name: 'Name',
  ...
  project_application: {
    workstation: {
      ...
      api_receiver_tip:
        'Note: Please confirm that "API update" in the API test task notification has been set "Intra-site message" message, otherwise, you will not receive message reminders',
      case_receiver_tip:
        'Note: Please confirm that "CASE update" in the API test task notification has been set "Intra-site message" message, otherwise, you will not receive message reminders',
      no_show_setting_tip:
        'After checking, the pop-up window will no longer pop up, and the synchronization and change notification will be performed according to the setting content by default: If you need to change the setting, you can open the setting item in the drop-down option next to the save button。',
      go_to_api_message: 'Interface API update message notification',
      go_to_case_message: 'CASE update message notification',
      update_case_tip:
        'When the CASE changes, the creator of the automation scene referring to the CASE will receive an in-site message reminder',
    },
  },
  task: {
