...
import { ITestPlanNode, getTestPlanNodePage, getTestPlanNodes } from '/@/apis/modules/test-plan-node-api'
...
const {
  ...
  (page, pageSize) => getTestPlanNodePage(page, pageSize, condition),
  {
    // 请求前的初始数据（接口返回的数据格式）
    initialData: {
      total: 0,
    ...
    total: (response: IPageResponse<ITestPlanNode>) => response.totalRow,
    data: (response: IPageResponse<ITestPlanNode>) => response.records,
...
const columns: DataTableColumns<ITestPlanNode> = [
  { title: '名称', key: 'name' },
  {
...
<template>
  ...
  <add-node ref="addNode" @refresh="parallelRequest" />
</template>
