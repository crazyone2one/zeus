...
import { ref } from 'vue'
import { IWorkspace, createWs, updateWorkspaceSpecial } from '/@/apis/modules/workspace-api'
...
const {
  ...
  form: model,
} = useForm((model) => (model.id ? updateWorkspaceSpecial(model) : createWs(model)), {
  // 初始化表单数据
  initialForm: {
    id: '',
    name: '',
...
const open = (row?: IWorkspace): void => {
